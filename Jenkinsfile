...
node {
  withEnv(['AZURE_SUBSCRIPTION_ID=9daf9d86-59fc-4bc1-bbcd-dece2f0115df',
    ...
    stage('deploy') {
      ...
      def ftpsProfile = getFtpPublishProfile pubProfilesJson
      // upload package
      sh "curl -T target/calculator-1.0.war $ftpsProfile.url/webapps/ROOT.war -u '$ftpsProfile.username:$ftpsProfile.password'" 
      
      // linha 38 - u -> '$ftpProfile.username:$ftpProfile.password'
      // $ftpProfile.devobackofficeuser:$ftpProfile.password'
      // log out
      sh 'az logout'
    }
  }
